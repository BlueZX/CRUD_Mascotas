...
class PuestoController{
    ...
    $aux = '';
		$rows = $puesto->getList();
    $aux .= "</h1><h2>puesto</h2><a class=\"btn btn-success\" href=\"?c=Puesto&a=addform\">Agregar</a><div class=\"table-responsive\"><table class=\"table table-striped\"><thead><tr><th>id</th><th>descripcion</th><th>Accion</th></tr></thead><tbody>";
    for($i=0;$i<count($rows);$i++){
      $aux .= "\t<tr>\n";
      foreach($rows[$i] as $campo=>$valor) {
        $puesto->$campo = $valor;
        $aux .= "\t\t<td>". $puesto->$campo . "</td>\n";
    }

      $aux .= "\t<td><a href=\"?c=Puesto&a=editform&id=" . $puesto->id . " \"class=\"btn btn-success\">Editar</a><a href=\"?c=Puesto&a=delete&id=" . $puesto->id . " \"class=\"btn btn-danger\">Eliminar</a></td></tr>\n";
    }
    $aux .= "</tbody></table>\n";
    echo $aux;
  ...
  function addform(){
    ...
    echo ' > Agregar</h1><h2>Agregar puesto:</h2>
      <form class="form-horizontal" action="?c=Puesto&a=add" method="post">
  ...
  function editform(){
    ...
    $aux = ' > Editar</h1><h2>Editar puesto:</h2>
      <form class="form-horizontal" action="?c=Puesto&a=edit" method="post">
        <div class="form-group">
          <label class="control-label col-sm-2">
            id:
          </label>
          <div class="col-sm-10">
            <input type="text" class="form-control"
              name="id" placeholder="id" value="' . $puesto->id . '"  readonly="readonly" onkeypress="return valida(event)"
            >
          </div>
        </div>
        ...
        <div class="form-group">
          ...
          <div class="col-sm-10">
            <input type="text" class="form-control"
              name="descripcion" placeholder="Descripcion" value="' . $puesto->descripcion . '"
            >
          </div>
        </div>
	...
	function nav(){
		echo '<nav class="col-sm-3 col-md-2 d-none d-sm-block sidebar">
          ...
          <ul class="nav nav-pills flex-column">
            ...
            <li class="nav-item">
              <a class="nav-link active" href="?c=Puesto&a=list">Puestos</a>
            </li>
          </ul>
